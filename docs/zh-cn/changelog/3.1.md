...
- [#7808](https://github.com/hyperf/hyperf/pull/7808) 优化 AMQP 消费者：当 `concurrent.limit` 小于 `qos.prefetch_count` 时，记录错误日志并自动扩大 `concurrent.limit`，避免触发 `AMQPInvalidFrameException` 异常。
